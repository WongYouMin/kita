...
- With destructuring
    ...
    - Note that not all property of the object have to be extracted
        ...
        console.log(membership, points);

        // Expected output: Gold, 1200
        ```

### Part 2: Object destructuring and Function
...
### Part 3: Renaming property in Object Destructuring
- Destructured property can be renamed to avoid variables with duplicated or confusing names
    ```
...
### Part 4: Object Destructuring in Nested objects
- Given the reward object below, extract its `name`, category's `type` and `active`
    ```
    const reward = {
...
### Part 5: Object Destructuring and Default Value
- Default value is used when the value obtained from the property is undefined (not exists or explicitly assigned with `undefined` value).
